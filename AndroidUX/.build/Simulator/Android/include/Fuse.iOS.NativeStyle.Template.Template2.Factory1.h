// This file was generated based on 'C:\ProgramData\Uno\Packages\Fuse.iOS\0.18.8\.cache\GeneratedCode\$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#pragma once
#include <Uno.Object.h>
#include <Uno.UX.IFactory.h>
namespace g{namespace Fuse{namespace iOS{struct NativeStyle__Template__Template2;}}}
namespace g{namespace Fuse{namespace iOS{struct NativeStyle__Template__Template2__Factory1;}}}

namespace g{
namespace Fuse{
namespace iOS{

// public partial sealed class NativeStyle.Template.Template2.Factory1 :53
// {
struct NativeStyle__Template__Template2__Factory1_type : uType
{
    ::g::Uno::UX::IFactory interface0;
};

NativeStyle__Template__Template2__Factory1_type* NativeStyle__Template__Template2__Factory1_typeof();
void NativeStyle__Template__Template2__Factory1__ctor__fn(NativeStyle__Template__Template2__Factory1* __this, ::g::Fuse::iOS::NativeStyle__Template__Template2* parent);
void NativeStyle__Template__Template2__Factory1__New1_fn(NativeStyle__Template__Template2__Factory1* __this, uObject** __retval);
void NativeStyle__Template__Template2__Factory1__New2_fn(::g::Fuse::iOS::NativeStyle__Template__Template2* parent, NativeStyle__Template__Template2__Factory1** __retval);

struct NativeStyle__Template__Template2__Factory1 : uObject
{
    uStrong< ::g::Fuse::iOS::NativeStyle__Template__Template2*> __parent1;

    void ctor_(::g::Fuse::iOS::NativeStyle__Template__Template2* parent);
    uObject* New1();
    static NativeStyle__Template__Template2__Factory1* New2(::g::Fuse::iOS::NativeStyle__Template__Template2* parent);
};
// }

}}} // ::g::Fuse::iOS
