...
set DATA_DIR=/data/data/com.Onboarding
set DEVICE_GDBSERVER=%DATA_DIR%/lib/gdbserver

set cmd="%ADB% shell ps | %AWK% -f %AWK_SCRIPTS%\extract-pid.awk -v PACKAGE=""com.Onboarding"""
FOR /F "delims=" %%i IN (' %cmd% ') DO SET PID=%%i

echo.
echo ## Starting gdbserver

start /b cmd /c %ADB% shell run-as com.Onboarding %DEVICE_GDBSERVER% +debug-socket --attach %PID%
timeout /t 1 > nul
