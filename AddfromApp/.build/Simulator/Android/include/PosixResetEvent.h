...
bool ResetEvent_WaitOne(void * state, int timeoutMillis)
{
	...
	uint64_t nanoseconds = (now.tv_sec * 1000000000) + (now.tv_usec * 1000);
	uint64_t timeoutNanoseconds = (timeoutMillis * 1000000) + nanoseconds;

	struct timespec timeout;
	timeout.tv_sec = (time_t)(timeoutNanoseconds / 1000000000);
	timeout.tv_nsec = (long)(timeoutNanoseconds % 1000000000);
