// This file was generated based on 'C:\ProgramData\Uno\Packages\Fuse.Animations\0.11.3\$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_FUSE_ANIMATIONS_MASTER_PROPERTY__FLOAT3_H__
#define __APP_FUSE_ANIMATIONS_MASTER_PROPERTY__FLOAT3_H__

#include <app/Fuse.Animations.IMixerMaster.h>
#include <app/Fuse.Animations.MasterBase__float3.h>
#include <app/Fuse.Animations.MasterPropertyGet.h>
#include <Uno.h>
namespace app { namespace Fuse { namespace Animations { struct MixerBase; } } }
namespace app { namespace Uno { namespace UX { struct Property__float3; } } }
namespace app { namespace Uno { struct Float3; } }

namespace app {
namespace Fuse {
namespace Animations {

struct MasterProperty__float3;

struct MasterProperty__float3__uType : ::app::Fuse::Animations::MasterBase__float3__uType
{
    ::app::Fuse::Animations::MasterPropertyGet __interface_1;
};

MasterProperty__float3__uType* MasterProperty__float3__typeof();

void MasterProperty__float3___ObjInit_1(MasterProperty__float3* __this, ::app::Uno::UX::Property__float3* property, ::app::Fuse::Animations::MixerBase* mixerBase);
::app::Uno::Float3 MasterProperty__float3__get_RestValue(MasterProperty__float3* __this);
::uObject* MasterProperty__float3__GetPropertyObject(MasterProperty__float3* __this);
void MasterProperty__float3__OnInactive(MasterProperty__float3* __this);

struct MasterProperty__float3 : ::app::Fuse::Animations::MasterBase__float3
{
    ::uStrong< ::app::Uno::UX::Property__float3*> Property;

    void _ObjInit_1(::app::Uno::UX::Property__float3* property, ::app::Fuse::Animations::MixerBase* mixerBase) { MasterProperty__float3___ObjInit_1(this, property, mixerBase); }
    ::uObject* GetPropertyObject() { return MasterProperty__float3__GetPropertyObject(this); }
};

}}}


#endif
