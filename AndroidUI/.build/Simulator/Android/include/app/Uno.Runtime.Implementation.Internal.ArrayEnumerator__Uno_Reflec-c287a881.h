// This file was generated based on 'C:\ProgramData\Uno\Packages\UnoCore\0.13.2\Source\Uno\Runtime\Implementation\Internal\$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_UNO_RUNTIME_IMPLEMENTATION_INTERNAL_ARRAY_ENUMERATOR__UNO_REFLEC_C287A881_H__
#define __APP_UNO_RUNTIME_IMPLEMENTATION_INTERNAL_ARRAY_ENUMERATOR__UNO_REFLEC_C287A881_H__

#include <app/Uno.Collections.IEnumerator.h>
#include <app/Uno.Collections.IEnumerator__Uno_Reflection_CppField.h>
#include <app/Uno.IDisposable.h>
#include <app/Uno.Object.h>
#include <Uno.h>

namespace app {
namespace Uno {
namespace Runtime {
namespace Implementation {
namespace Internal {

struct ArrayEnumerator__Uno_Reflection_CppField;

struct ArrayEnumerator__Uno_Reflection_CppField__uType : ::uClassType
{
    ::app::Uno::Collections::IEnumerator__Uno_Reflection_CppField __interface_0;
    ::app::Uno::IDisposable __interface_1;
    ::app::Uno::Collections::IEnumerator __interface_2;
};

ArrayEnumerator__Uno_Reflection_CppField__uType* ArrayEnumerator__Uno_Reflection_CppField__typeof();

void ArrayEnumerator__Uno_Reflection_CppField___ObjInit(ArrayEnumerator__Uno_Reflection_CppField* __this, ::uArray* source);
void ArrayEnumerator__Uno_Reflection_CppField__Dispose(ArrayEnumerator__Uno_Reflection_CppField* __this);
::uField* ArrayEnumerator__Uno_Reflection_CppField__get_Current(ArrayEnumerator__Uno_Reflection_CppField* __this);
bool ArrayEnumerator__Uno_Reflection_CppField__MoveNext(ArrayEnumerator__Uno_Reflection_CppField* __this);
ArrayEnumerator__Uno_Reflection_CppField* ArrayEnumerator__Uno_Reflection_CppField__New_1(::uStatic* __this, ::uArray* source);
void ArrayEnumerator__Uno_Reflection_CppField__Reset(ArrayEnumerator__Uno_Reflection_CppField* __this);

struct ArrayEnumerator__Uno_Reflection_CppField : ::uObject
{
    ::uStrong< ::uArray*> _source;
    int _iterator;
    ::uField* _current;

    void _ObjInit(::uArray* source) { ArrayEnumerator__Uno_Reflection_CppField___ObjInit(this, source); }
    void Dispose() { ArrayEnumerator__Uno_Reflection_CppField__Dispose(this); }
    ::uField* Current() { return ArrayEnumerator__Uno_Reflection_CppField__get_Current(this); }
    bool MoveNext() { return ArrayEnumerator__Uno_Reflection_CppField__MoveNext(this); }
    void Reset() { ArrayEnumerator__Uno_Reflection_CppField__Reset(this); }
};

}}}}}


#endif
