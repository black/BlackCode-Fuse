// This file was generated based on 'C:\ProgramData\Uno\Packages\UnoCore\0.13.2\Source\Uno\Runtime\Implementation\Internal\$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_UNO_RUNTIME_IMPLEMENTATION_INTERNAL_ARRAY_ENUMERATOR__UNO_NET_I_P_END_POINT_H__
#define __APP_UNO_RUNTIME_IMPLEMENTATION_INTERNAL_ARRAY_ENUMERATOR__UNO_NET_I_P_END_POINT_H__

#include <app/Uno.Collections.IEnumerator.h>
#include <app/Uno.Collections.IEnumerator__Uno_Net_IPEndPoint.h>
#include <app/Uno.IDisposable.h>
#include <app/Uno.Object.h>
#include <Uno.h>
namespace app { namespace Uno { namespace Net { struct IPEndPoint; } } }

namespace app {
namespace Uno {
namespace Runtime {
namespace Implementation {
namespace Internal {

struct ArrayEnumerator__Uno_Net_IPEndPoint;

struct ArrayEnumerator__Uno_Net_IPEndPoint__uType : ::uClassType
{
    ::app::Uno::Collections::IEnumerator__Uno_Net_IPEndPoint __interface_0;
    ::app::Uno::IDisposable __interface_1;
    ::app::Uno::Collections::IEnumerator __interface_2;
};

ArrayEnumerator__Uno_Net_IPEndPoint__uType* ArrayEnumerator__Uno_Net_IPEndPoint__typeof();

void ArrayEnumerator__Uno_Net_IPEndPoint___ObjInit(ArrayEnumerator__Uno_Net_IPEndPoint* __this, ::uArray* source);
void ArrayEnumerator__Uno_Net_IPEndPoint__Dispose(ArrayEnumerator__Uno_Net_IPEndPoint* __this);
::app::Uno::Net::IPEndPoint* ArrayEnumerator__Uno_Net_IPEndPoint__get_Current(ArrayEnumerator__Uno_Net_IPEndPoint* __this);
bool ArrayEnumerator__Uno_Net_IPEndPoint__MoveNext(ArrayEnumerator__Uno_Net_IPEndPoint* __this);
ArrayEnumerator__Uno_Net_IPEndPoint* ArrayEnumerator__Uno_Net_IPEndPoint__New_1(::uStatic* __this, ::uArray* source);
void ArrayEnumerator__Uno_Net_IPEndPoint__Reset(ArrayEnumerator__Uno_Net_IPEndPoint* __this);

struct ArrayEnumerator__Uno_Net_IPEndPoint : ::uObject
{
    ::uStrong< ::uArray*> _source;
    int _iterator;
    ::uStrong< ::app::Uno::Net::IPEndPoint*> _current;

    void _ObjInit(::uArray* source) { ArrayEnumerator__Uno_Net_IPEndPoint___ObjInit(this, source); }
    void Dispose() { ArrayEnumerator__Uno_Net_IPEndPoint__Dispose(this); }
    ::app::Uno::Net::IPEndPoint* Current() { return ArrayEnumerator__Uno_Net_IPEndPoint__get_Current(this); }
    bool MoveNext() { return ArrayEnumerator__Uno_Net_IPEndPoint__MoveNext(this); }
    void Reset() { ArrayEnumerator__Uno_Net_IPEndPoint__Reset(this); }
};

}}}}}


#endif
