// This file was generated based on 'C:\ProgramData\Uno\Packages\Fuse.Triggers\0.11.3\Actions\$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_FUSE_TRIGGERS_ACTIONS_SET__FUSE_DRAWING_DYNAMIC_BRUSH_H__
#define __APP_FUSE_TRIGGERS_ACTIONS_SET__FUSE_DRAWING_DYNAMIC_BRUSH_H__

#include <app/Fuse.Triggers.Actions.TriggerAction.h>
#include <Uno.h>
namespace app { namespace Fuse { namespace Drawing { struct DynamicBrush; } } }
namespace app { namespace Fuse { namespace Internal { struct Blender__Fuse_Drawing_DynamicBrush; } } }
namespace app { namespace Fuse { struct Node; } }
namespace app { namespace Uno { namespace UX { struct Property__Fuse_Drawing_DynamicBrush; } } }

namespace app {
namespace Fuse {
namespace Triggers {
namespace Actions {

struct Set__Fuse_Drawing_DynamicBrush;

struct Set__Fuse_Drawing_DynamicBrush__uType : ::app::Fuse::Triggers::Actions::TriggerAction__uType
{
};

Set__Fuse_Drawing_DynamicBrush__uType* Set__Fuse_Drawing_DynamicBrush__typeof();

void Set__Fuse_Drawing_DynamicBrush___ObjInit_1(Set__Fuse_Drawing_DynamicBrush* __this, ::app::Uno::UX::Property__Fuse_Drawing_DynamicBrush* target);
::uDelegate* Set__Fuse_Drawing_DynamicBrush__get_Expression(Set__Fuse_Drawing_DynamicBrush* __this);
::app::Fuse::Drawing::DynamicBrush* Set__Fuse_Drawing_DynamicBrush__get_Increment(Set__Fuse_Drawing_DynamicBrush* __this);
::app::Uno::UX::Property__Fuse_Drawing_DynamicBrush* Set__Fuse_Drawing_DynamicBrush__get_Target(Set__Fuse_Drawing_DynamicBrush* __this);
bool Set__Fuse_Drawing_DynamicBrush__get_UpdateRestValue(Set__Fuse_Drawing_DynamicBrush* __this);
::app::Fuse::Drawing::DynamicBrush* Set__Fuse_Drawing_DynamicBrush__get_Value(Set__Fuse_Drawing_DynamicBrush* __this);
Set__Fuse_Drawing_DynamicBrush* Set__Fuse_Drawing_DynamicBrush__New_1(::uStatic* __this, ::app::Uno::UX::Property__Fuse_Drawing_DynamicBrush* target);
void Set__Fuse_Drawing_DynamicBrush__Perform(Set__Fuse_Drawing_DynamicBrush* __this, ::app::Fuse::Node* target);
void Set__Fuse_Drawing_DynamicBrush__set_Expression(Set__Fuse_Drawing_DynamicBrush* __this, ::uDelegate* value);
void Set__Fuse_Drawing_DynamicBrush__set_Increment(Set__Fuse_Drawing_DynamicBrush* __this, ::app::Fuse::Drawing::DynamicBrush* value);
void Set__Fuse_Drawing_DynamicBrush__set_Target(Set__Fuse_Drawing_DynamicBrush* __this, ::app::Uno::UX::Property__Fuse_Drawing_DynamicBrush* value);
void Set__Fuse_Drawing_DynamicBrush__set_UpdateRestValue(Set__Fuse_Drawing_DynamicBrush* __this, bool value);
void Set__Fuse_Drawing_DynamicBrush__set_Value(Set__Fuse_Drawing_DynamicBrush* __this, ::app::Fuse::Drawing::DynamicBrush* value);
void Set__Fuse_Drawing_DynamicBrush__Update(Set__Fuse_Drawing_DynamicBrush* __this, ::app::Fuse::Drawing::DynamicBrush* value);

struct Set__Fuse_Drawing_DynamicBrush : ::app::Fuse::Triggers::Actions::TriggerAction
{
    bool _updateRestValue;
    ::uStrong< ::app::Fuse::Drawing::DynamicBrush*> _increment;
    bool _hasIncrement;
    ::uStrong< ::app::Fuse::Internal::Blender__Fuse_Drawing_DynamicBrush*> _blender;
    ::uStrong< ::app::Uno::UX::Property__Fuse_Drawing_DynamicBrush*> _Target;
    ::uStrong< ::app::Fuse::Drawing::DynamicBrush*> _Value;
    ::uStrong< ::uDelegate*> _Expression;

    void _ObjInit_1(::app::Uno::UX::Property__Fuse_Drawing_DynamicBrush* target) { Set__Fuse_Drawing_DynamicBrush___ObjInit_1(this, target); }
    ::uDelegate* Expression() { return Set__Fuse_Drawing_DynamicBrush__get_Expression(this); }
    ::app::Fuse::Drawing::DynamicBrush* Increment() { return Set__Fuse_Drawing_DynamicBrush__get_Increment(this); }
    ::app::Uno::UX::Property__Fuse_Drawing_DynamicBrush* Target() { return Set__Fuse_Drawing_DynamicBrush__get_Target(this); }
    bool UpdateRestValue() { return Set__Fuse_Drawing_DynamicBrush__get_UpdateRestValue(this); }
    ::app::Fuse::Drawing::DynamicBrush* Value() { return Set__Fuse_Drawing_DynamicBrush__get_Value(this); }
    void Expression(::uDelegate* value) { Set__Fuse_Drawing_DynamicBrush__set_Expression(this, value); }
    void Increment(::app::Fuse::Drawing::DynamicBrush* value) { Set__Fuse_Drawing_DynamicBrush__set_Increment(this, value); }
    void Target(::app::Uno::UX::Property__Fuse_Drawing_DynamicBrush* value) { Set__Fuse_Drawing_DynamicBrush__set_Target(this, value); }
    void UpdateRestValue(bool value) { Set__Fuse_Drawing_DynamicBrush__set_UpdateRestValue(this, value); }
    void Value(::app::Fuse::Drawing::DynamicBrush* value) { Set__Fuse_Drawing_DynamicBrush__set_Value(this, value); }
    void Update(::app::Fuse::Drawing::DynamicBrush* value) { Set__Fuse_Drawing_DynamicBrush__Update(this, value); }
};

}}}}


#endif
