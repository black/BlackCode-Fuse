// This file was generated based on 'C:\ProgramData\Uno\Packages\UnoCore\0.13.2\Source\Uno\Collections\$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_UNO_COLLECTIONS_LIST1_ENUMERATOR__UNO_EVENT_HANDLER_H__
#define __APP_UNO_COLLECTIONS_LIST1_ENUMERATOR__UNO_EVENT_HANDLER_H__

#include <app/Uno.Collections.IEnumerator.h>
#include <app/Uno.Collections.IEnumerator__Uno_EventHandler.h>
#include <app/Uno.IDisposable.h>
#include <app/Uno.Object.h>
#include <Uno.h>
namespace app { namespace Uno { namespace Collections { struct List__Uno_EventHandler; } } }

namespace app {
namespace Uno {
namespace Collections {

struct List1_Enumerator__Uno_EventHandler;

struct List1_Enumerator__Uno_EventHandler__uType : ::uStructType
{
    ::app::Uno::Collections::IEnumerator__Uno_EventHandler __interface_0;
    ::app::Uno::IDisposable __interface_1;
    ::app::Uno::Collections::IEnumerator __interface_2;
};

List1_Enumerator__Uno_EventHandler__uType* List1_Enumerator__Uno_EventHandler__typeof();

void List1_Enumerator__Uno_EventHandler___ObjInit(List1_Enumerator__Uno_EventHandler* __this, ::app::Uno::Collections::List__Uno_EventHandler* source);
void List1_Enumerator__Uno_EventHandler__Dispose(List1_Enumerator__Uno_EventHandler* __this);
::uDelegate* List1_Enumerator__Uno_EventHandler__get_Current(List1_Enumerator__Uno_EventHandler* __this);
bool List1_Enumerator__Uno_EventHandler__MoveNext(List1_Enumerator__Uno_EventHandler* __this);
List1_Enumerator__Uno_EventHandler List1_Enumerator__Uno_EventHandler__New_1(::uStatic* __this, ::app::Uno::Collections::List__Uno_EventHandler* source);
void List1_Enumerator__Uno_EventHandler__Uno_Collections_IEnumerator_Reset(List1_Enumerator__Uno_EventHandler* __this);

struct List1_Enumerator__Uno_EventHandler
{
    ::uStrong< ::app::Uno::Collections::List__Uno_EventHandler*> _source;
    int _version;
    int _iterator;
    ::uStrong< ::uDelegate*> _current;

    void _ObjInit(::app::Uno::Collections::List__Uno_EventHandler* source) { List1_Enumerator__Uno_EventHandler___ObjInit(this, source); }
    void Dispose() { List1_Enumerator__Uno_EventHandler__Dispose(this); }
    ::uDelegate* Current() { return List1_Enumerator__Uno_EventHandler__get_Current(this); }
    bool MoveNext() { return List1_Enumerator__Uno_EventHandler__MoveNext(this); }
};

}}}


#endif
