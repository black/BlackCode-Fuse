// This file was generated based on 'C:\ProgramData\Uno\Packages\UnoCore\0.13.2\Source\Uno\Collections\$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_UNO_COLLECTIONS_LIST1_ENUMERATOR__FUSE_RESOURCES_I_MEMORY_RESOURCE_H__
#define __APP_UNO_COLLECTIONS_LIST1_ENUMERATOR__FUSE_RESOURCES_I_MEMORY_RESOURCE_H__

#include <app/Uno.Collections.IEnumerator.h>
#include <app/Uno.Collections.IEnumerator__Fuse_Resources_IMemoryResource.h>
#include <app/Uno.IDisposable.h>
#include <app/Uno.Object.h>
#include <Uno.h>
namespace app { namespace Uno { namespace Collections { struct List__Fuse_Resources_IMemoryResource; } } }

namespace app {
namespace Uno {
namespace Collections {

struct List1_Enumerator__Fuse_Resources_IMemoryResource;

struct List1_Enumerator__Fuse_Resources_IMemoryResource__uType : ::uStructType
{
    ::app::Uno::Collections::IEnumerator__Fuse_Resources_IMemoryResource __interface_0;
    ::app::Uno::IDisposable __interface_1;
    ::app::Uno::Collections::IEnumerator __interface_2;
};

List1_Enumerator__Fuse_Resources_IMemoryResource__uType* List1_Enumerator__Fuse_Resources_IMemoryResource__typeof();

void List1_Enumerator__Fuse_Resources_IMemoryResource___ObjInit(List1_Enumerator__Fuse_Resources_IMemoryResource* __this, ::app::Uno::Collections::List__Fuse_Resources_IMemoryResource* source);
void List1_Enumerator__Fuse_Resources_IMemoryResource__Dispose(List1_Enumerator__Fuse_Resources_IMemoryResource* __this);
::uObject* List1_Enumerator__Fuse_Resources_IMemoryResource__get_Current(List1_Enumerator__Fuse_Resources_IMemoryResource* __this);
bool List1_Enumerator__Fuse_Resources_IMemoryResource__MoveNext(List1_Enumerator__Fuse_Resources_IMemoryResource* __this);
List1_Enumerator__Fuse_Resources_IMemoryResource List1_Enumerator__Fuse_Resources_IMemoryResource__New_1(::uStatic* __this, ::app::Uno::Collections::List__Fuse_Resources_IMemoryResource* source);
void List1_Enumerator__Fuse_Resources_IMemoryResource__Uno_Collections_IEnumerator_Reset(List1_Enumerator__Fuse_Resources_IMemoryResource* __this);

struct List1_Enumerator__Fuse_Resources_IMemoryResource
{
    ::uStrong< ::app::Uno::Collections::List__Fuse_Resources_IMemoryResource*> _source;
    int _version;
    int _iterator;
    ::uStrong< ::uObject*> _current;

    void _ObjInit(::app::Uno::Collections::List__Fuse_Resources_IMemoryResource* source) { List1_Enumerator__Fuse_Resources_IMemoryResource___ObjInit(this, source); }
    void Dispose() { List1_Enumerator__Fuse_Resources_IMemoryResource__Dispose(this); }
    ::uObject* Current() { return List1_Enumerator__Fuse_Resources_IMemoryResource__get_Current(this); }
    bool MoveNext() { return List1_Enumerator__Fuse_Resources_IMemoryResource__MoveNext(this); }
};

}}}


#endif
