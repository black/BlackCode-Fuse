// This file was generated based on 'C:\ProgramData\Uno\Packages\UnoCore\0.13.2\Source\Uno\Runtime\Implementation\Internal\$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_UNO_RUNTIME_IMPLEMENTATION_INTERNAL_ARRAY_ENUMERATOR__UNO_U_X_RESOURCE_H__
#define __APP_UNO_RUNTIME_IMPLEMENTATION_INTERNAL_ARRAY_ENUMERATOR__UNO_U_X_RESOURCE_H__

#include <app/Uno.Collections.IEnumerator.h>
#include <app/Uno.Collections.IEnumerator__Uno_UX_Resource.h>
#include <app/Uno.IDisposable.h>
#include <app/Uno.Object.h>
#include <Uno.h>
namespace app { namespace Uno { namespace UX { struct Resource; } } }

namespace app {
namespace Uno {
namespace Runtime {
namespace Implementation {
namespace Internal {

struct ArrayEnumerator__Uno_UX_Resource;

struct ArrayEnumerator__Uno_UX_Resource__uType : ::uClassType
{
    ::app::Uno::Collections::IEnumerator__Uno_UX_Resource __interface_0;
    ::app::Uno::IDisposable __interface_1;
    ::app::Uno::Collections::IEnumerator __interface_2;
};

ArrayEnumerator__Uno_UX_Resource__uType* ArrayEnumerator__Uno_UX_Resource__typeof();

void ArrayEnumerator__Uno_UX_Resource___ObjInit(ArrayEnumerator__Uno_UX_Resource* __this, ::uArray* source);
void ArrayEnumerator__Uno_UX_Resource__Dispose(ArrayEnumerator__Uno_UX_Resource* __this);
::app::Uno::UX::Resource* ArrayEnumerator__Uno_UX_Resource__get_Current(ArrayEnumerator__Uno_UX_Resource* __this);
bool ArrayEnumerator__Uno_UX_Resource__MoveNext(ArrayEnumerator__Uno_UX_Resource* __this);
ArrayEnumerator__Uno_UX_Resource* ArrayEnumerator__Uno_UX_Resource__New_1(::uStatic* __this, ::uArray* source);
void ArrayEnumerator__Uno_UX_Resource__Reset(ArrayEnumerator__Uno_UX_Resource* __this);

struct ArrayEnumerator__Uno_UX_Resource : ::uObject
{
    ::uStrong< ::uArray*> _source;
    int _iterator;
    ::uStrong< ::app::Uno::UX::Resource*> _current;

    void _ObjInit(::uArray* source) { ArrayEnumerator__Uno_UX_Resource___ObjInit(this, source); }
    void Dispose() { ArrayEnumerator__Uno_UX_Resource__Dispose(this); }
    ::app::Uno::UX::Resource* Current() { return ArrayEnumerator__Uno_UX_Resource__get_Current(this); }
    bool MoveNext() { return ArrayEnumerator__Uno_UX_Resource__MoveNext(this); }
    void Reset() { ArrayEnumerator__Uno_UX_Resource__Reset(this); }
};

}}}}}


#endif
